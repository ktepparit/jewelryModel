...
# Function 4: Product Content Writer (UPDATED!)
def generate_full_product_content(api_key, img_pil, raw_input):
    url = f"[https://generativelanguage.googleapis.com/v1beta/](https://generativelanguage.googleapis.com/v1beta/){MODEL_TEXT_SEO}:generateContent?key={api_key}"
    
    # Fill prompt with Raw Input
...
# === TAB 3: AI PRODUCT WRITER (UPDATED!) ===
with tab3:
                        ...
                        if data:
                            ...
                            for idx, item in enumerate(img_seo_list):
                                with st.container():
                                    ...
                                    if idx == 0 and writer_img: cols[0].image(writer_img, width=50)
                                    else: cols[0].write(f"#{idx+1}")
