...
# --- 2. CLOUD DATABASE FUNCTIONS (Cleaned & Fixed) ---
def get_prompts():
    if "JSONBIN_API_KEY" in st.secrets and "JSONBIN_BIN_ID" in st.secrets:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Logic ที่ถูกต้องในการแกะข้อมูล (แก้ปัญหา library หาย)
...
def list_available_models(api_key):
    try:
        res = requests.get(f"[https://generativelanguage.googleapis.com/v1beta/models?key=](https://generativelanguage.googleapis.com/v1beta/models?key=){api_key}")
        return res.json().get("models", []) if res.status_code == 200 else None
    except: return None
...
with st.sidebar:
    ...
    if "JSONBIN_API_KEY" in st.secrets:
        st.caption("✅ Cloud DB Active")
        # โชว์จำนวน Item ที่โหลดมาได้ เพื่อความมั่นใจ
        st.caption(f"📚 {len(st.session_state.library)} styles loaded.")
    else:
