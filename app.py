    ...
    if "JSONBIN_API_KEY" in st.secrets and "JSONBIN_BIN_ID" in st.secrets:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # ตรวจสอบโครงสร้างข้อมูลเพื่อป้องกันความผิดพลาด
                if isinstance(data, list):
                    return data
                elif isinstance(data, dict) and "record" in data:
                    return data["record"]
                else:
                    st.sidebar.error(f"DB Format Unknown: {type(data)}")
                    return DEFAULT_PROMPTS
            else:
                st.sidebar.error(f"DB Fetch Error {response.status_code}: {response.text}")
                return DEFAULT_PROMPTS
        except Exception as e:
            st.sidebar.error(f"DB Exception: {e}")
            return DEFAULT_PROMPTS
    else:
        return DEFAULT_PROMPTS
...
def save_prompts(data):
    if "JSONBIN_API_KEY" in st.secrets and "JSONBIN_BIN_ID" in st.secrets:
        ...
        st.warning("Cannot save: No Database credentials found in Secrets.")
...
# Function: Safe Image Loader (ป้องกัน App Crash)
def safe_st_image(url, width=None):
    try:
        ...
        # แสดง Error เล็กๆ แทนที่จะ Crash ทั้งแอพ
        st.caption(f"⚠️ Img Load Error")
...
with st.sidebar:
    ...
    # --- DEBUG STATUS (เช็คการเชื่อมต่อ Database) ---
    st.divider()
    st.write("📊 **DB Status**")
    if "JSONBIN_API_KEY" in st.secrets:
        st.caption("✅ Cloud Configured")
        # โชว์จำนวนข้อมูลที่โหลดมาได้จริง
        st.caption(f"📚 Prompts Loaded: {len(st.session_state.library)}")
        
        # ปุ่ม Force Reload (เผื่อข้อมูลไม่อัพเดต)
...
with tab1:
    ...
    with c2:
        ...
        cats = list(set(p.get('category', 'Uncategorized') for p in lib)) if lib else []
